...
description: This skill should be used when setting up, adopting, migrating, or refreshing a lightweight `.context/` directory for project/session continuity, especially when the user mentions file-based context, context management, PROJECT.md, STEERING.md, TASKS.md, AGENTS.md intake rules, or agent instruction files.
---
...
- Adding agent instruction file rules that tell future agents which context files to read.
...
Add or update the repository's existing agent instruction file. Prefer the file the project already uses instead of creating a parallel convention.
...
- `CLAUDE.md`
- `GEMINI.md`
- another project-specific agent instruction file
...
If the repository already has a context-intake section, merge instead of duplicating. If no agent instruction file exists, ask before creating one.
...
| Agent behavior rule changes | the active agent instruction file |
...
3. Update the active agent instruction file with required context intake.
...
| Creating a parallel agent instruction file | Update the project's existing agent instruction file. |
| Forgetting required context intake | Add required read order. |
